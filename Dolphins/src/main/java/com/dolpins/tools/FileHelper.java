...
public class FileHelper {
	...
	public static File getFileByClassLoader(String resourceName) throws IOException {
		Enumeration<URL> urls = ReadProperties.class.getClassLoader().getResources(resourceName);
		while (urls.hasMoreElements()) {
		    String path = URLDecoder.decode(urls.nextElement().getFile(), "UTF-8");
	...
	static {
	    ...
	    loadBinaryExtentionsList("cn/org/rapid_framework/generator/util/binary_filelist.txt",true);
	}
	
	public static void loadBinaryExtentionsList(String resourceName,boolean ignoreException) {
	    try {
	        InputStream input  = ReadProperties.class.getClassLoader().getResourceAsStream(resourceName);
